...
def _media_type_label(media) -> str:
    if media.media_type == MT_ALBUM:
        return "carousel"
    if media.media_type == MT_VIDEO:
        return "video"
    return "image"
...
def media_to_metadata(media, list_name: str | None, media_files: list[dict]) -> dict:
    ...
    # Collect accessibility captions from the top-level post and from each
    # carousel item, deduplicated while preserving order.
    raw_alts = [getattr(media, "accessibility_caption", None)]
    if media.media_type == MT_ALBUM and media.resources:
        raw_alts.extend(getattr(r, "accessibility_caption", None)
        ...
        if media.media_type == MT_ALBUM:
            for i, r in enumerate(media.resources, 1):
                if r.media_type == MT_VIDEO:
                    ...
                    _move(Path(p), "image")
        elif media.media_type == MT_VIDEO:
...
def do_fetch_post(cl: Client, shortcode: str, output_dir: str,
    ...
    # MediaNotFound is allowed to propagate: the serve loop will report
    # its original type to Node so callers can distinguish it from
    # transient errors.
