...
export function createProgress(total, label) {
  ...
  const counts = { added: 0, existing: 0, skipped: 0, removed: 0, error: 0 };
  let done = 0;

  function rate() {
    const elapsedMin = (Date.now() - startedAt) / 60_000;
    return elapsedMin > 0 ? done / elapsedMin : 0;
  }

  function eta() {
    const r = rate();
    if (r <= 0 || done === 0) return '—';
    const remaining = Math.max(0, total - done);
    const seconds = (remaining / r) * 60;
    return fmtDuration(seconds);
  }

  function renderTTY() {
    const pct = total > 0 ? Math.floor((done / total) * 100) : 100;
    const summary = `+${counts.added} ~${counts.existing} ·${counts.skipped} !${counts.error}`;
  ...
  return {
    ...
    finish() {
      ...
      const summary = `+${counts.added} ~${counts.existing} ·${counts.skipped} -${counts.removed} !${counts.error}`;
